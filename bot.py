...
def provide_number(message):
    if re.match(pattern=r'^[0-9E]\d{3} [ABEIKMHOPCTX]{2}-[1-7]$', string=str(message.text).upper()):
        ...
        bot.register_next_step_handler(message=msg, callback=provide_brand, car_number=str(message.text).upper())
    else:
        msg = bot.send_message(chat_id=message.chat.id, text="Не удалось распознать номер. Попробуйте снова.")
        bot.register_next_step_handler(message=msg, callback=provide_number)


def provide_brand(message, car_number):
    session = database.create_db_connection()()
...
def search_result(message):
    ...
    if len(result) > 0:
        ...
        for el in result:
            ...
            inline_keyboard.add(InlineKeyboardButton(text='Отправить сообщение о блокировке', callback_data=el.user_id))
...
def send_block_auto_message(message):
    bot.send_message(chat_id=message.data, text='Ваш автомобиль блокирует выезд')
